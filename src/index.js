...
async function upsertCustomerProfile(env, customerId, name, extra = {}) {
  try {
    ...
    if (extra.diseno || extra.zona || extra.tamano || extra.analisis || extra.estilo) {
      await env.DB.prepare(`
        UPDATE customer_profiles SET
          ultimo_diseno = COALESCE(?, ultimo_diseno),
          ultima_zona = COALESCE(?, ultima_zona),
          ultimo_tamano = COALESCE(?, ultimo_tamano),
          ultimo_analisis = COALESCE(?, ultimo_analisis),
          ultimo_estilo = COALESCE(?, ultimo_estilo)
        WHERE customer_id = ?
      `).bind(extra.diseno||null, extra.zona||null, extra.tamano||null, extra.analisis||null, extra.estilo||null, customerId).run();
    }
...
function getDashboardHTML() {
  const html = [
    ...
    'async function generarMockupAdmin(desc,zona,estilo){const btn=event.target;btn.disabled=true;btn.textContent="⏳ Generando...";const box=document.getElementById("chat-box");try{const r=await fetch("/api/generate-mockup",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({descripcion:desc,zona:zona,estilo:estilo,tamano_cm:10})});const d=await r.json();if(d.ok&&d.image_url){const div=document.createElement("div");div.className="chat-msg-bot";div.innerHTML="<div style=\"padding:8px 0\"><div style=\"color:#00f5ff;font-size:.8em;margin-bottom:6px\">🎨 MOCKUP GENERADO</div><img src=\""+d.image_url+"\" style=\"width:100%;border-radius:8px;border:1px solid #9d00ff\" loading=\"lazy\"/></div>";box.appendChild(div);box.scrollTop=box.scrollHeight;btn.textContent="✅ Generado";}else{btn.disabled=false;btn.textContent="🎨 Generar Mockup";}}catch(e){btn.disabled=false;btn.textContent="🎨 Generar Mockup";}}',
    '<\/script>',
    '</body></html>'
  ].join('\n');
