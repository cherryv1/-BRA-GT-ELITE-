...
function getDashboardHTML() {
  const html = [
    ...
    '<select id="voice-select" style="background:rgba(0,10,30,.9);border:1px solid rgba(0,245,255,.3);color:#00f5ff;border-radius:20px;padding:6px 10px;font-size:.75em;cursor:pointer;max-width:130px;">''<option>🔊 Voz...</option></select>',
    '<select id="voice-select" style="background:rgba(0,10,30,.9);border:1px solid rgba(0,245,255,.3);color:#00f5ff;border-radius:20px;padding:6px 10px;font-size:.75em;cursor:pointer;max-width:130px;"><option>🔊 Voz...</option></select>',
    ...
    'window.speechSynthesis.onvoiceschanged=function(){const voices=window.speechSynthesis.getVoices().filter(v=>v.lang.startsWith("es"));const sel=document.getElementById("voice-select");if(!sel)return;sel.innerHTML=voices.map((v,i)=>"<option value="+i+">"+v.name+"</option>").join("");};window.getSelectedVoice=function(){const sel=document.getElementById("voice-select");const voices=window.speechSynthesis.getVoices().filter(v=>v.lang.startsWith("es"));return voices[sel?sel.selectedIndex:0]||null;};',
    '<\/script>',
    '</body></html>'
  ].join('\n');
